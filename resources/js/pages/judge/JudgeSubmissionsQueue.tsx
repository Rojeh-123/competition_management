...
function JudgeSubmissionsQueue() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <Head title="Submissions Queue – Evaluation Entries" />
      <Navbar />
                ...
                <tbody>
                  {visibleSubmissions.map((sub) => (
                    <tr key={sub.id} className="border-t hover:bg-muted/50">
                      <td className="px-4 py-3 font-mono text-muted-foreground whitespace-nowrap">#SUB-{String(sub.id).padStart(3, '0')}</td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap"><Button size="sm" className="cursor-pointer" onClick={() => router.visit(route('judge.review', { id: sub.id }))}>Evaluate</Button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
